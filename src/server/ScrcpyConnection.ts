...
export class ScrcpyConnection extends Mw {
    ...
    private async startWithReverseTunnel(options: ScrcpyOptions): Promise<net.Socket[]> {
        ...
        // socket to that port. scrcpy-server connects out (2 or 3 sockets) — we accept.
        ...
        // With audio disabled, scrcpy connects video then control. Insert the
        // same synthetic AUDIO_DISABLED socket used by the forward-tunnel path
        // so callers can keep the stable [video, audio, control] layout.
